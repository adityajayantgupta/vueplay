    ...
    <div class="row">
      ...
      <div id="songs">
        <div class="large-2 columns song" v-for="song in songs">  
          <img class="cover" v-bind:src="song.image[2]['#text']">
...
<script>
  ...
  export default {
    ...
    beforeCreate() {
      ...
      .then (response => {
        for (let i in response.data.tracks.track) {
          axios.get('http://ws.audioscrobbler.com/2.0/?method=track.getInfos&format=json&api_key=' + credentials.apikey)
        }
        this.songs = response.data.tracks.track
      })
      .catch(e => console.log(e))
    }
  }
